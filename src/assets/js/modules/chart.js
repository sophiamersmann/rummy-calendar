      ...
      if (this.active.tip.type != 'year') {
        ...
        if (this.active.tip.type == 'week') {
          const xi = elem.attr('x'),
            wi = elem.attr('width');
          others = others.filter((d, j, m) =>
            xi != d3.select(m[j]).attr('x') || wi != d3.select(m[j]).attr('width'));

          // all but the selected month
        } else if (this.active.tip.type == 'month') {
          others = others.filter((d, j) =>
            formatDate(d3.utcMonth(d)) !=
            formatDate(d3.utcMonth(date)));
        }

        others = others.attr('fill-opacity', 0.5);
      }
...
function filterScores(data, activePlayers) {
  ...
  const isValid = d3.zip(...activeScores.map(d => d.scores)).map(d => d.every(x => x !== null));
